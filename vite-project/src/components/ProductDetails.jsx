...
function ProductDetails() {
    ...
    useEffect(() => {
        let filteredProduct = products.length
            ...
            : JSON.parse(localStorage.getItem("products"))?.find(p => p.id.toString() === productId);

        if (!filteredProduct) return; // Prevents crashes if no data is found
        
        const priceInRupees = Math.floor(filteredProduct.price * 80);
        const discount = Math.floor(priceInRupees * (filteredProduct.discountPercentage / 100));
        const grandTotal = priceInRupees - discount;

        setProductAmount(priceInRupees);
        setDiscountAmount(discount);
        setGrandTotalAmount(grandTotal);

        setAddedToCart(cartItems.some(item => item.id === filteredProduct.id));
        setProduct(filteredProduct);
    }, [productId, products, cartItems]); // Ensuring dependencies update correctly

    const starRatings = ["⭐", "⭐⭐", "⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐⭐"];

    const handleAddToCart = () => {
        dispatch(addToCart({ ...product, quantity: 1 }));
        dispatch(updateOrderAmount({ totalAmount: productAmount, totalDiscount: discountAmount, grandTotal: grandTotalAmount }));
        setAddedToCart(true);
    };

    if (!product) return <h1 className="mt-28">Loading Product...</h1>;

    return (
        <section className="mt-10 min-[500px]:mt-20 px-5 flex flex-col min-[700px]:flex-row items-center gap-x-5 pt-[40px] ">
            <figure>
                <img src={product.images[0]} className="max-w-[300px] min-[300px] rounded-xl" alt={product.title} />
            </figure>
            <section>
                <h1 className="text-3xl min-[500px]:text-4xl font-bold">{product.title}</h1>
                <div className="mt-3">
                    {product.tags.map(tag => (
                        <span key={tag} className="mr-2 text-xs font-semibold bg-purple-700 text-white py-1 px-3 rounded-full">
                            {tag.toUpperCase()}
                        </span>
                    ))}
                </div>
                <p className="mt-3 font-semibold">
                    Rating: <span>{starRatings[Math.floor(product.rating) - 1]}</span> 
                    <span className="ml-2">({product.reviews.length} Reviews)</span>
                </p>
                <p className="flex items-center gap-x-2 mt-3 text-xl font-semibold">
                    <FontAwesomeIcon icon={faIndianRupee} /> {grandTotalAmount}
                    <span className="text-sm mt-1">({product.discountPercentage}% OFF)</span>
                </p>
                <section className="mt-5 font-semibold text-lg">
                    Description:
                    <ul className="mt-2 ml-5 block space-y-2 max-w-[600px] text-base">
                        {product.description.split(". ").map(sentence => (
                            <li key={sentence} className="list-disc">{sentence}</li>
                        ))}
                    </ul>
                </section>
                <button 
                    ...
                    className={`mt-7 font-semibold text-lg bg-purple-700 text-white py-2 px-4 rounded-lg ${addedToCart ? "opacity-60 cursor-not-allowed" : "opacity-100 cursor-pointer"}`}
                >
